...
class iZoneLightDevice extends Device {
  ...
  async onInit() {
    ...
    this.registerCapabilityListener("dim", async (value) => {
      const light = this.getThisLight();
      if (light == undefined) return;
      await this.homey.app.sendSimpleiLightCmd("LiBright", { No: light.Index, Bright: value });
      this.homey.app.pausePolling(500);
    });
    ...
    this.registerCapabilityListener("light_temperature", async (value) => {
      const light = this.getThisLight();
      if (light == undefined) return;
      await this.homey.app.sendSimpleiLightCmd("LiTemp", { No: light.Index, Temp: value });
      this.homey.app.pausePolling(500);
    });
